...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			...
			{
				$user_image = make_avatar(strtoupper($_POST["user_profile"][0]));
			}
...
function upload_image()
{
	...
	{
		$extension = explode('.', $_FILES['user_profile']['name']);
		$new_name = rand() . '.' . $extension[1];
		$destination = 'images/' . $new_name;
		move_uploaded_file($_FILES['user_profile']['tmp_name'], $destination);
		return $destination;
	}
}
